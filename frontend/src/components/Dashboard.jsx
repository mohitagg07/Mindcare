...
function Gauge({ score }) {
  ...
  const color = score>=0.75?'#FF6B6B':score>=0.5?'#FFB547':score>=0.25?'#60A5FA':'#56CFB2'
    ...
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center' }}>
      <div style={{ position:'relative', width:190, height:100 }}>
        <svg width="190" height="100" viewBox="0 0 190 100">
          <path d="M 16 96 A 79 79 0 0 1 174 96" fill="none" stroke="rgba(255,255,255,0.05)" strokeWidth="14" strokeLinecap="round"/>
        ...
        <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column', alignItems:'center', paddingTop:32 }}>
          <span style={{ fontSize:34, fontWeight:800, color:'#F5F0FF', letterSpacing:'-1px', lineHeight:1 }}>
  ...
  return (
    <div>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:5 }}>
        <span style={{ fontSize:12, color:'#8B8AAA' }}>{label}</span>
...
export default function Dashboard() {
  ...
  if (!riskData && !phq9Result && !gad7Result) return (
    <div style={{ height:'100%', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center',
      gap:24, padding:40, textAlign:'center', background:'#13111C' }}>
      <div style={{ width:72, height:72, borderRadius:22, background:'rgba(155,109,255,0.1)',
        border:'1px solid rgba(155,109,255,0.2)', display:'flex', alignItems:'center', justifyContent:'center',
        boxShadow:'0 0 40px rgba(155,109,255,0.12)' }}>
        <LayoutDashboard size={32} color="#9B6DFF"/>
      </div>
      <div>
        <h3 style={{ fontSize:22, fontWeight:700, color:'#F5F0FF', margin:0 }}>Dashboard is empty</h3>
  ...
  return (
    <div style={{ height:'100%', overflowY:'auto', background:'#13111C' }}>
      <div style={{ maxWidth:960, margin:'0 auto', padding:'28px 24px', display:'flex', flexDirection:'column', gap:18 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:14 }}>
          <div style={{ width:44, height:44, borderRadius:13, flexShrink:0,
            ...
            <LayoutDashboard size={20} color="#9B6DFF"/>
          </div>
          <div>
        ...
        {/* Info */}
        <div style={{ display:'flex', gap:10, padding:'12px 16px', borderRadius:14,
          background:'rgba(155,109,255,0.06)', border:'1px solid rgba(155,109,255,0.15)' }}>
          <Info size={13} color="#9B6DFF" style={{ flexShrink:0, marginTop:1 }}/>
          <p style={{ fontSize:12, color:'#8B8AAA', lineHeight:1.65, margin:0 }}>
            Risk score = <strong style={{ color:'#C4A3FF' }}>PHQ-9 40%</strong> + <strong style={{ color:'#C4A3FF' }}>GAD-7 25%</strong> + <strong style={{ color:'#C4A3FF' }}>Facial 20%</strong> + <strong style={{ color:'#C4A3FF' }}>Chat Mood 15%</strong>. The EMA trajectory engine tracks trends over time and adapts AI responses accordingly.
          </p>
        </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 2fr', gap:16 }}>
          <div className="card" style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:10 }}>
            <p style={{ fontSize:11, color:'#4A4870', fontWeight:700, textTransform:'uppercase',
              letterSpacing:'0.8px', alignSelf:'flex-start' }}>Overall Risk</p>
            <Gauge score={riskData?.risk_score??0}/>
          </div>
          <div className="card" style={{ display:'flex', flexDirection:'column', gap:14 }}>
            <p style={{ fontSize:11, color:'#4A4870', fontWeight:700, textTransform:'uppercase', letterSpacing:'0.8px' }}>
              Component Breakdown
            </p>
            {riskData?.components ? (
              <>
                <Bar label="PHQ-9 — Depression (40%)"  value={riskData.components.phq9_contribution}   color="#9B6DFF"/>
                <Bar label="GAD-7 — Anxiety (25%)"      value={riskData.components.gad7_contribution}   color="#56CFB2"/>
                <Bar label="Facial Emotion (20%)"        value={riskData.components.facial_contribution} color="#60A5FA"/>
                <Bar label="Chat Mood (15%)"             value={riskData.components.text_contribution}   color="#FFB547"/>
              </>
        ...
        {trajectory && (
          <div className="card">
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:16 }}>
              <p style={{ fontSize:11, color:'#4A4870', fontWeight:700, textTransform:'uppercase',
                letterSpacing:'0.8px', margin:0 }}>Mental State Trajectory (72h EMA)</p>
              {trajectory.trend && (
            ...
            {recentPoints.length > 1 ? (
              <ResponsiveContainer width="100%" height={160}>
                <LineChart data={recentPoints} margin={{ top:5, right:20, bottom:5, left:0 }}>
                  ...
                  <Tooltip contentStyle={{ background:'#1D1A2C', border:'1px solid rgba(155,109,255,0.2)', borderRadius:10, fontSize:12 }}
                    labelStyle={{ color:'#8B8AAA' }} formatter={v=>[`${v}% risk`,'EMA State']}/>
                  <Line type="monotone" dataKey="ema" stroke="#9B6DFF" strokeWidth={2.5}
                    dot={{ fill:'#9B6DFF', r:3 }} activeDot={{ r:5 }}/>
                </LineChart>
              </ResponsiveContainer>
            ) : (
              <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                {[['Current EMA', trajectory.current_ema!=null ? `${Math.round(trajectory.current_ema*100)}%` : '—', '#9B6DFF'],
                  ...
                  ['Data Points', trajectory.points ?? 0, '#8B8AAA']].map(([l,v,c]) => (
                  <div key={l} style={{ flex:1, padding:'14px', borderRadius:12, textAlign:'center',
                    background:'#1D1A2C', border:`1px solid ${c}20` }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:14 }}>
          {[
            { label:'PHQ-9',    icon:Activity,    result:phq9Result, max:27,  special:null },
            { label:'GAD-7',    icon:ShieldAlert, result:gad7Result, max:21,  special:null },
            { label:'EMA State',icon:Brain,       result:null,       max:null, special:trajectory },
          ].map(({ label, icon:Icon, result, max, special }) => (
            <div key={label} className="card" style={{ textAlign:'center', padding:'20px 16px' }}>
              <Icon size={15} color="#4A4870" style={{ margin:'0 auto 10px' }}/>
              <p style={{ fontSize:11, color:'#4A4870', marginBottom:8, fontWeight:600,
                textTransform:'uppercase', letterSpacing:'0.5px' }}>{label}</p>
              {result ? (
                <>
                  <p style={{ fontSize:28, fontWeight:800, color:'#F5F0FF', margin:0 }}>
                ...
                <>
                  <p style={{ fontSize:26, fontWeight:800, color:'#F5F0FF', margin:0 }}>
        ...
        {(phq9Result || gad7Result) && (
          <div className="card">
            <p style={{ fontSize:11, color:'#4A4870', fontWeight:700, textTransform:'uppercase',
              letterSpacing:'0.8px', marginBottom:16 }}>Wellbeing Radar</p>
            <ResponsiveContainer width="100%" height={200}>
              <RadarChart data={radarData} margin={{ top:10, right:24, bottom:10, left:24 }}>
                ...
                <Radar name="Risk" dataKey="value" stroke="#9B6DFF" fill="#9B6DFF" fillOpacity={0.15} strokeWidth={2}/>
              </RadarChart>
            </ResponsiveContainer>
          </div>
        )}
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
            <div className="card" style={{ display:'flex', flexDirection:'column', gap:14 }}>
              <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                <Dumbbell size={14} color="#56CFB2"/>
                <p style={{ fontSize:14, fontWeight:700, color:'#D4CEE8', margin:0 }}>Exercises</p>
              </div>
              {recommendations.exercises.map((ex,i) => (
                <div key={i} style={{ display:'flex', gap:12 }}>
                  <div style={{ width:22, height:22, borderRadius:7, flexShrink:0, marginTop:1,
                    ...
                    display:'flex', alignItems:'center', justifyContent:'center',
                    fontSize:10, fontWeight:800, color:'#56CFB2' }}>{i+1}</div>
                  <p style={{ fontSize:13, color:'#8B8AAA', lineHeight:1.65, margin:0 }}>{ex}</p>
                </div>
              ))}
            </div>
            <div className="card" style={{ display:'flex', flexDirection:'column', gap:14 }}>
              <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                ...
                <p style={{ fontSize:14, fontWeight:700, color:'#D4CEE8', margin:0 }}>Resources</p>
              </div>
              <p style={{ fontSize:13, color:'#8B8AAA', lineHeight:1.65, padding:'10px 14px', borderRadius:10,
              ...
              {recommendations.resources.map((r,i) => {
                ...
                return (
                  <div key={i} style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 14px',
                    ...
                    <div style={{ width:32, height:32, borderRadius:10, background:'rgba(255,181,71,0.1)',
                      border:'1px solid rgba(255,181,71,0.2)', display:'flex', alignItems:'center',
                      justifyContent:'center', flexShrink:0 }}>
                      <Icon size={13} color="#FFB547"/>
                    </div>
                    <div>
                      <p style={{ fontSize:12, fontWeight:700, color:'#D4CEE8', margin:0 }}>{r.name}</p>
        ...
        {riskData?.risk_level==='HIGH' && (
          <div className="animate-scale-in" style={{ display:'flex', gap:14, padding:'16px 18px', borderRadius:16,
            ...
            <div>
              <p style={{ fontSize:14, fontWeight:700, color:'#FF9999', marginBottom:6 }}>High Risk Detected</p>
              <p style={{ fontSize:13, color:'#FF9999', opacity:0.85, lineHeight:1.65 }}>
